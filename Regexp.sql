...
-- Replacing brackets ( ) using regexp_replace()
SELECT REGEXP_REPLACE('Customers - (NY)','\\(|\\)','') AS customers;


-- extracting the date out of a string
SELECT TRIM(REGEXP_REPLACE(string, '[a-z/-/A-Z/.]', '')) AS date_value 
FROM (SELECT 'My DOB is 04-12-1976.' AS string) a;


-- extracting the date out of a string and the output should be in date format
SELECT TO_DATE(TRIM(REGEXP_REPLACE(string, '[a-z/-/A-Z/.]', '')),'dd-mm-yyyy') AS date_value 
FROM (SELECT 'My DOB is 04-12-1976.' AS string) a;


-- another way of doing the above extraction:
SELECT REGEXP_REPLACE('My DOB is 04-12-1976.', '[a-z,A-Z/.]', '');
...
SELECT REGEXP_REPLACE('It was the best of times, it was the worst of times', '( ){1,}','') AS "result";
...
-- Validate if date is in a valid format using REGEXP_LIKE function
...
/*
REGULAR EXPRESSIONS - FROM SNOWFLAKE DOCUMENTATION

Regular Expressions are string functions
...
SELECT
  email,
  REGEXP_LIKE(email,'\\w+@[a-zA-Z0-9-]+\.[a-zA-Z0-9-.]+') AS is_valid
FROM
  (SELECT
...
SELECT REGEXP_LIKE('9809182006','[89][0-9]{9}');

SELECT REGEXP_LIKE('Zxrg5z','[A-Z][a-z]*[0-9][a-z*]');


-- REGEXP for EMAIL matching
SELECT REGEXP_LIKE('bar@example.com','^([A-Za-z0-9_\\.-]+)@([A-Za-z0-9_\\.-]+)\\.[a-zA-z]{2,5}$');
SELECT REGEXP_LIKE('bar.619klm@example.org','^([A-Za-z0-9_\\.-]+)@([A-Za-z0-9_\\.-]+)\\.[a-zA-z]{2,5}$');
SELECT REGEXP_LIKE('bar@example.co.in','^([A-Za-z0-9_\\.-]+)@([A-Za-z0-9_\\.-]+)\\.[a-zA-z]{2,5}$');
